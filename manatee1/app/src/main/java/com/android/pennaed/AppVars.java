		    ...
		    public void done(List<ParseObject> aedList, ParseException e) {
		        if (e == null) {
		            ...
		            for(ParseObject aedParse : aedList){
		            	AED aed = new AED(aedParse.getObjectId(),aedParse.getString("name"),aedParse.getDouble("latitude"),aedParse.getDouble("longitude"),aedParse.getString("address"),aedParse.getString("inBuildingDirections"),aedParse.getString("contactName"),aedParse.getInt("contactPhone"),aedParse.getUpdatedAt(),aedParse.getParseFile("photoFile"), integerId);
